...
public class UserEmailsDAOImpl implements UserEmailsDAO {
	...
	public void setDataSource(DataSource dataSource) {
		...
		this.saveUserEmailPrc = new SimpleJdbcCall(dataSource).withCatalogName("PKG_EMAIL").withProcedureName("PRC_SAVE_USER_EMAIL");
	}
	...
	public UserEmail saveUserEmail(UserEmail userEmail) {
		...
		SqlParameterSource in = new MapSqlParameterSource().
				...
				addValue("IN_USER_ID", userEmail.getUserId()).addValue("IN_SUBJECT", userEmail.getSubject())
				.addValue("IN_MESSAGE", userEmail.getMessage()).addValue("IN_SENT_TO", userEmail.getSentTo())
				.addValue("IN_SENT_CC", userEmail.getSentCC()).addValue("IN_SENT_BCC", userEmail.getSentBCC());
